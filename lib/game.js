...
// Retrieve dimensions
  ...
  enactSetupSequence() {
    ...
    let playerTexture = PIXI.Texture.fromImage('./assets/sprites/forward.png')
    this.player = new PlayerCharacter(playerTexture, [0,0], this)
    while (this.player === false) {
      console.log('not yet');
    }
    [this.player.x, this.player.y] = [300, 300]


    let sparkyTexture = PIXI.Texture.fromImage('./assets/sprites/sparky_sparky_lightning_man/sparky_man_test_0.png');
    let sparkMan = new SparkySparkyLightningMan(sparkyTexture, [1,0], this);
    [sparkMan.x, sparkMan.y] = [200, 200]
    this.enemies.addChild(sparkMan);
    let sparkMan2 = new SparkySparkyLightningMan(sparkyTexture, [0, -1], this);
    [sparkMan2.x, sparkMan2.y] = [500, 200]
  ...
  makeSomeBunnies() {
    ...
    while (this.bunnies.children.length < 10) {
      let bunny = new MovingObject(texture, [1, 1]);
      [bunny.anchor.x, bunny.anchor.y] = [0.5, 0.5];
