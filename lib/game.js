...
// Retrieve dimensions
  ...
  enactSetupSequence () {
    ...
    let wallTexture = new PIXI.Texture.fromImage('assets/sprites/test_wall_segment.png');
    let wall = this.buildAWall(wallTexture);
  ...
  buildAWall (wallTexture) {
    let wall = [];
    let topLeft = [25, 25];
    let currentPos = topLeft;
    while (currentPos[1] < Game.dim_y - 25) {
      wall.push(new WallSegment(currentPos, wallTexture));
      currentPos[1] += 50;
    }

    while (currentPos[0] < Game.dim_x - 25) {
      wall.push(new WallSegment(currentPos, wallTexture));
      currentPos[0] += 50;
    }

    while (currentPos[1] > 25) {
      wall.push(new WallSegment(currentPos, wallTexture));
      currentPos[1] -= 50;
    }

    while (currentPos[0] > 25) {
      wall.push(new WallSegment(currentPos, wallTexture));
      currentPos[0] -= 50;
    }
    return wall;
