...
class Dynamite extends PIXI.Container {
  ...
  animateDynamite () {
    ...
    if (i === 26)  {
      ...
      for (let j = 0; j < this.game.enemies.children.length; j++) {
        this.checkCollision(this.game.enemies.children[j])
      }
      for (let j = 0; j < this.game.players.children.length; j++) {
        this.checkCollision(this.game.players.children[j])
      }
    } else if (i >= 33) {
