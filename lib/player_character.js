...
class PlayerCharacter extends PIXI.Container {
  ...
  leaveRoom () {
    if (this.x > Game.dim_x) {
      this.game.currentRoom.moveToNextRoom("right", this.game);
      this.x = this.width;
    } else if (this.x < 0) {
      this.game.currentRoom.moveToNextRoom("left", this.game);
      this.x = Game.dim_x - this.width;
    } else if (this.y > Game.dim_y) {
      this.game.currentRoom.moveToNextRoom("down", this.game);
      this.y = this.height;
    } else {
      this.game.currentRoom.moveToNextRoom("up", this.game);
      this.y = Game.dim_y - this.height;
    }
    this.leftRoom = false;
  }
