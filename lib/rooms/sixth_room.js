...
const SixthRoom = {
  ...
  setUpRoom () {
    this.createFloor();
    new Cultist([250, 600], this.game);
    new Cultist([450, 300], this.game);
    new Cultist([650, 600], this.game);
  ...
  createFloor () {
    ...
    this.game.floor.addChild(backgroundSprite);
    let nextToWall = 40;
    let xIncrement = 10;
    let yIncrement = 6;
    ...
    while (currentPos[0] < Game.dim_x / 2 - (6 * xIncrement)) {
      ...
      for (let j = 0; j < tiles.length; j++) {
        this.game.floor.addChild(tiles[j])
      }
    ...
    this.game.floor.addChild(circleSprite);
