...
const PlayerObject = {
  ...
  update (object) {
    ...
    for (let i = 0; i < object.game.items.length; i++) {
      if ((object.game.items[i].x - object.x) < object.game.items[i].width && (object.game.items[i].x - object.x) > -(object.game.items[i].width) &&
       (object.game.items[i].y - object.y) < object.game.items[i].height && (object.game.items[i].y - object.y) > -(object.game.items[i].height)) {
         if (SAT.testPolygonPolygon(object.game.items[i].hitBox, object.hitBox)) {
           object.game.items[i].pickUp(object);
         }
      }
    }
  },
