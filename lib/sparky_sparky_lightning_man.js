...
class SparkySparkyLightningMan extends MovingObject {
  constructor(texture, velocity, game) {
    super(texture, velocity);
    this.tracker = {};
    this.anchor.x = 0.5;
    this.anchor.y = 0.5;
    this.attackTexture = new PIXI.Texture.fromImage('./assets/sprites/sparky_sparky_lightning_man/sparky_man_test_1.png');
    this.health = 100;
    this.attackDelay = 0;
  ...
  createLightning(target) {
    ...
    this.lightning = new PIXI.Sprite(warningTexture);
    this.game.masterContainer.addChild(this.lightning);
    this.lightning.anchor.x = 0.5;
    this.lightning.anchor.y = 0.5;
    this.lightning.height = 1000;
    ...
    this.lightning.x = xDisplacement * ratio + this.x;
    this.lightning.y = yDisplacement * ratio + this.y;
    ...
    this.attackBorders = new PIXI.Graphics();
    this.game.masterContainer.addChild(this.attackBorders);
  }
  ...
  animateLightning() {
    ...
    function blank () {
      ...
      that.lightning.texture = blankTexture;
    }
    switch(this.lightningCounter) {
      case 60:
        ...
        break;
      case 62:
        let firstLightning = PIXI.utils.TextureCache.lightning1;
        this.lightning.texture = firstLightning;
        break;
      case 64:
        blank();
        break;
      case 66:
        let secondLightning = PIXI.utils.TextureCache.lightning2;
        this.lightning.texture = secondLightning;
        break;
      case 68:
        blank();
        break;
      case 70:
        let thirdLightning = PIXI.utils.TextureCache.lightning3;
        this.lightning.texture = thirdLightning;
        console.log(this.lightning.getBounds());
        if (this.checkHit()) {
          this.game.player.takeDamage(40);
        }
        break;
      case 80:
        this.game.masterContainer.removeChild(this.lightning);
        this.game.masterContainer.removeChild(this.attackBorders);
        this.lightningCounter = 0;
        this.lightning = false;
        this.attacking = false;
        this.texture = PIXI.Texture.fromImage('./assets/sprites/sparky_sparky_lightning_man/sparky_man_test_0.png');
  ...
  attack(target) {
    this.texture = this.attackTexture;
    this.attacking = true;
    this.attackDelay = 0;
