...
class Murkspawn extends PIXI.Container {
  ...
  animateMovement () {
    if (this.animationFrame % 10 === 0 && (this.velX !== 0 || this.velY !== 0)) {
      let frame = (this.animationFrame / 10);
      this.sprite.texture = this.textures["move" + frame];
    }
  }
  ...
  attack (target) {
    this.attacking = true;
    this.sprite.texture = this.textures["attack" + Math.floor(this.animationFrame / 10)];
